...
from jormungandr.core import C51Agent, PrioritizedReplayBuffer, RunningNormalizer, TBLogger
...
class JormungandrRuntime:
    """Thread-safe Jormungandr runtime for multi-model training over HTTP."""
    ...
    def _parse_learner_config(raw: Optional[Mapping[str, Any]]) -> Optional[LearnerConfig]:
        ...
        cfg = LearnerConfig(
            ...
            algo=str(raw.get("algo", "c51")).strip().lower() or "c51",
        ...
        if cfg.algo != "c51":
            raise JormungandrServiceError(f"unsupported learner.algo: {cfg.algo} (service supports c51 today)")
        return cfg
    ...
    @classmethod
    def _agent_weight_summary(cls, agent: Optional[C51Agent]) -> JsonDict:
        if agent is None:
            return {}
        summary: JsonDict = {}
        try:
            summary["q"] = cls._parameter_stats_from_tensors(list(agent.q.parameters()))
        except Exception:
            summary["q"] = {}
        aux_head = getattr(agent, "aux_head", None)
        if aux_head is not None:
            try:
                summary["aux"] = cls._parameter_stats_from_tensors(list(aux_head.parameters()))
            except Exception:
                summary["aux"] = {}
        return summary
    ...
    def _build_learner_state(obs_dim: int, cfg: Optional[LearnerConfig]) -> Optional[LearnerState]:
        ...
        agent = C51Agent(
            obs_dim=int(obs_dim),
            num_actions=len(cfg.action_values),
            action_values=[float(x) for x in cfg.action_values],
            hidden=int(cfg.hidden),
            aux_hidden=int(cfg.aux_hidden),
            lr=float(cfg.lr),
            gamma=float(cfg.gamma),
            v_min=float(cfg.v_min),
            v_max=float(cfg.v_max),
            atoms=int(cfg.atoms),
            target_update=int(cfg.target_update),
            max_grad_norm=float(cfg.max_grad),
            device=str(cfg.device),
            aux_classes=int(cfg.aux_classes) if bool(cfg.aux_enabled) else 0,
            aux_weight=float(cfg.aux_weight),
            aux_class_weighting=str(cfg.aux_class_weighting),
            aux_label_smoothing=float(cfg.aux_label_smoothing),
        )
        return LearnerState(cfg=cfg, agent=agent)
    ...
    def _build_restored_learner_state(
        ...
        *,
        ...
        agent = _build_agent_from_checkpoint(ckpt, spec=spec, device=str(cfg.device))
        learner = LearnerState(cfg=cfg, agent=agent)
            ...
            try:
                for _ in range(int(cfg.updates_per_tick)):
                    ...
                    with rec.lock:
                        ...
                        batch_raw, idxs, weights = rec.replay.sample(bs, beta)
    ...
    def _replay_step_key(cls, episode_id: Any, timestep: Any) -> Optional[str]:
        """Return the compatibility key used by pre-Jormungandr clients."""
    ...
    def policy_infer(
        ...
        *,
        ...
        with rec.lock:
            ...
            with learner.agent_lock:
                action, action_idx = learner.agent.act(
                    obs_model_np,
                    epsilon=max(0.0, float(epsilon)),
                    deterministic=bool(deterministic),
                )
                with torch.no_grad():
                    dist = learner.agent.q.dist(obs_t)
                    q_vals = learner.agent._q_values(dist).squeeze(0).detach().cpu().numpy().tolist()
                    aux_probs = None
                    aux_pred = None
            ...
            out: JsonDict = {
                ...
                "q_values": [float(x) for x in q_vals],
                "action_values": list(learner.cfg.action_values),
                "policy_version": int(learner.policy_version),
                "updates": int(learner.updates),
            }
        ...
        *,
        ...
        with rec.lock:
            ...
            with learner.agent_lock:
                with torch.no_grad():
                    dist = learner.agent.q.dist(obs_t)
                    q_vals_t = learner.agent._q_values(dist)
                    q_vals = q_vals_t.detach().cpu().numpy()
                    greedy_idxs = np.argmax(q_vals, axis=1).astype(int)
                    aux_probs_arr = None
                    aux_preds = None
                    if getattr(learner.agent, "aux_head", None) is not None:
                        logits = learner.agent.aux_head(obs_t)
            ...
            for i, greedy_idx in enumerate(greedy_idxs.tolist()):
                idx = int(greedy_idx)
                if not bool(deterministic) and eps > 0.0 and np.random.rand() < eps:
                    idx = int(np.random.randint(0, len(action_values)))
                item: JsonDict = {
                    "action": float(action_values[idx]),
                    "action_idx": int(idx),
                    "q_values": [float(x) for x in q_vals[i].tolist()],
                }
...
class JormungandrHttpHandler(BaseHTTPRequestHandler):
    server_version = "JormungandrRuntime/0.1"
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Jormungandr multi-model runtime service")
