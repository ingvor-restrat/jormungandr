...
GAE weight before optimization, so a critic fitting one common outcome---or a
terminal residual with negligible reach at early decisions---is not mistaken
for a policy receiving useful credit.
...
ranking.  An independent three-seed CartPole diagnostic gives the built-in PPO
...
the implemented contracts and their limits.  A representation-parity control
...
factor groups rather than application-specific labels.
...
It pads only at the model collation boundary, serves one centrally versioned
...
nested inside rather than expanded into separate reward-bearing samples.
...
earlier factor's distribution; post-hoc projection is therefore rejected
rather than mislabeled as on-policy data.  The service validates complete
...
handler rejects unsupported or malformed content encodings.  This changes
transport size, not PPO semantics.  Complete episodes are implemented;
correctly bootstrapped partial
fragments and durable graph artifact references remain future extensions.

Structured behavior cloning uses a third, explicitly reward-free contract.
...
source/factor-group summaries without gradients.  Candidate IDs, rather than
row positions, define the target, so a candidate permutation does not relabel
...
The v2 structured profile exposes the same observability contract rather than
only its latest scalar.  Its metrics endpoint retains learner update history,
...
\begin{figure}[H]
  ...
  the three held-out run means.  The curve is a wiring benchmark, not a
  library-wide performance ranking.}
...
not any application's balance-group definition or downstream learning gain.
...
preference conditioning rather than masking, PPO credit, or horizon length.
This is a narrow capability test, not evidence of performance on a larger
joint-action problem.
...
construction.  Changing worker count changes nested factor count, not the
three trajectory records or the single nonzero reward.
...
enough for complete enumeration and isolates the structured formulation; it is
not evidence for larger combinatorial environments.

\subsection{Masked-action representation diagnostic}

Taxi-v4 isolates the legal-mask contract on 500 discrete states and six
...
and also motivates checkpoint selection and stability diagnostics; it is not
evidence that masks alone solve a complex application.
...
selects $\lambda=1$ for a terminal-only child experiment; it is an estimator
decision, not a learning or variance claim.
...
turn.  Exact masks remain environment inputs.  The additive prefix summary is
not a recurrent policy and does not provide temporal memory across turns.
...
Replay data are not durable across restart.  Structured PPO currently consumes
complete episodes rather than bootstrapped partial fragments.  The constrained
workbench is a small exact-oracle benchmark and does not establish scaling to a
large action system.  Application actors still own dynamic feasibility
callbacks and action execution.
...
advantage remain outside the release.
